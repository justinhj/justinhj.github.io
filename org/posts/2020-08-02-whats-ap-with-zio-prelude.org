...
exercising the new prelude types. If you want to see how flatMap can be written
stay tuned.
...
CovariantSubset instead of Covariant".
...
Future.sequence from the Scala standard library.
...
val ios = NonEmptyList(6,5,2,1,3,8,4,7).map(delayedPrintNumber)
// ios: NonEmptyList[ZIO[Console with Clock,String,Int]]
#+END_SRC

From my previous post you will know that we eventually get from sequence to a
more generalised function, traverse, of which sequence is a special case. In
prelude we also have traverse, and as you might guess, the operation of traverse
depends on which prelude data types we pass to it.
...
Questions: why is there no IdentityBoth for ZIO so I cannot traverse an empty
list of ZIO.
Why is there no way to do commutativeboth with NonEmptyTraversable?

In Cats you can traverse an empty list of Cats Effect no problem.
