...
After that I was able to TSUpdate and TSInstall whatever I needed without any issues and syntax highlighting worked along with it.
...
Since completion is now built-in I can able with the following global settings, and a special function required to enable it for each lsp server, if it supports it.
