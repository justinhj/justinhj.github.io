...
In particular I talk about k-way merge, a type of external sort, and four different algorithms and data structures, each having its own interactive application you can use to step through the algorithm and get a good understanding of its operation.
...
1. Populate a fixed size min-heap with the first (smallest) elements from the k runs.
...
There is still some room for improvement thanks to an ingenius idea from the 1950's.
